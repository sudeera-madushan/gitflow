...
class ItemRoutes {
  ...
  routeRequest(req, res, method, urlParts) {
    // GET /api/items
    if (method === "GET" && urlParts.length === 2 && urlParts[0] === "api" && urlParts[1] === "items") {
      this.controller.getAllItems(req, res);
    }
    // GET /api/items/:id
    else if (method === "GET" && urlParts.length === 3 && urlParts[0] === "api" && urlParts[1] === "items") {
      req.params = { id: urlParts[2] };
      this.controller.getItemById(req, res);
    }
    // POST /api/items
    else if (method === "POST" && urlParts.length === 2 && urlParts[0] === "api" && urlParts[1] === "items") {
      this.controller.createItem(req, res);
    }
    // PUT /api/items/:id
    else if (method === "PUT" && urlParts.length === 3 && urlParts[0] === "api" && urlParts[1] === "items") {
      req.params = { id: urlParts[2] };
      this.controller.updateItem(req, res);
    }
    // DELETE /api/items/:id
    else if (method === "DELETE" && urlParts.length === 3 && urlParts[0] === "api" && urlParts[1] === "items") {
      req.params = { id: urlParts[2] };
